...
- Everything is public — team management, all viewing (schedule, leaderboard), and match-result entry (recording/correcting a winner). Anyone can edit match results; there's no protection against mistakes or cheating.
- The leaderboard updates via simple polling, not websockets/SSE.
- No user accounts, no passwords, no auth of any kind anywhere in the app.
...
- **Auth:** None. Every route, including match-result entry, is open with no password, token, or session of any kind.
...
All routes under `/api`. No route requires authentication — everything, including match-result mutations, is open.
...
**Match results**
- `PUT /api/matches/:id/result` — body `{winner_team_number}`; validates winner is one of the match's two teams; `400` on invalid winner.
...
Key files: `src/app/main.tsx`, `App.tsx` (router + bottom nav), `pages/TeamsPage.tsx`, `SchedulePage.tsx`, `LeaderboardPage.tsx`, `components/TeamForm.tsx`, `TeamCard.tsx`, `ColorPicker.tsx`, `RoundCard.tsx`, `MatchRow.tsx`, `ScoreEntryModal.tsx`, `lib/api.ts`.
...
### Stage 3 — Match result recording
- **Goal:** anyone can record/correct a match winner, no login or password required.
- **Files:** `worker/routes/matches.ts` (`PUT`/`DELETE /api/matches/:id/result`), `ScoreEntryModal.tsx`.
- **Verify:** pick a winner → result persists across reload; a bad `winner_team_number` → `400`; correcting a result (re-recording or clearing) works with no prompt of any kind.
...
- **Files:** Tailwind/CSS pass, bottom-nav polish, loading/error states, `README.md` with `wrangler d1 create` / migrations deployment steps.
